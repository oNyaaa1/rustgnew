...
function SWEP:UpdateDown()
	...
	if pl:IsSprinting() and pl.CanSprint and pl:GetVelocity():LengthSqr() > pl:GetWalkSpeed() ^ 2 then
		self.DownProgress = Lerp(FrameTime() / self.DownSpeed, self.DownProgress, 1)
		self.WeaponDown = true
	else
		self.DownProgress = Lerp(FrameTime() / self.DownSpeed, self.DownProgress, 0)
...
function SWEP:DoWeaponBob(pos, ang, intensity)
	...
	if pl:GetVelocity() ~= Vector() then
		...
		if pl:IsSprinting() and pl.CanSprint and Velocity > pl:GetWalkSpeed() then Mul = 1 end
